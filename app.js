...
function renderVoucher(){
  ...
  if(btnConfirm) {
    btnConfirm.innerText = currentMode === 'proforma' ? 'Enviar Proforma' : 'Finalizar Compra';
...
function confirmarCompra() {
  ...
  voucherFechaTexto = voucherFechaTexto || fechaTexto;
  
  let distributed = 0;
  const productos = CARRITO.map((item, index) => {
    const precioUnit = item.cant >= 12 ? item.docena : item.unidad;
    const lineTotal = precioUnit * item.cant;
    const precio = index === CARRITO.length - 1
      ? Math.round((totales.subtotal - distributed) * 100) / 100
      : Math.round((lineTotal * totales.factor) * 100) / 100;
    distributed += precio;
    return {
      id: item.id || '',
...
function enviarProforma() {
  ...
  const productos = CARRITO.map(item => ({
    ...
    color: item.color,
    cantidad: item.cant,
    precioUnitario: item.cant >= 12 ? item.docena : item.unidad,
    precioVenta: (item.cant >= 12 ? item.docena : item.unidad) * totales.factor,
    totalProducto: ((item.cant >= 12 ? item.docena : item.unidad) * totales.factor) * item.cant
  }));
...
function descargarPDFVoucher() {
  ...
  const usuario = localStorage.getItem('usuario');
  const profile = userProfiles[usuario] || { nombre: usuario, celular: '+51922464915' };
  const numRef = ventaGuardada?.codigoRef || voucherReference || generarReferencia(currentMode);
  ...
  CARRITO.forEach(item => {
    ...
    pdf.setFontSize(9);
    pdf.text(`ID: ${item.id || '-'} - ${nombreTrunc}`, 10, y);
    y += 4;
    pdf.text(`Talla ${item.talla || '-'} - Color ${item.color || '-'} - Cant ${item.cant} - Precio S/ ${totalItem.toFixed(2)}`, 10, y);
    y += 6;
    pdf.text('----------------------------------------------------------------------', 105, y, { align: 'center' });
    y += 6;
  });
...
function mostrarVentanaWhatsappConPDF(){
  ...
  previewHtml += `CODIGO: ${voucherReference || (window.ultimoJSONVenta?.codigoRef || '-') }<br><br>`;
  ...
  CARRITO.forEach(item => {
    ...
    const precioEfectivo = precioUnit * totales.factor;
    const totalItem = precioEfectivo * item.cant;
    previewHtml += `ID: ${item.id || '-'} - ${item.producto}<br>`;
    previewHtml += `Talla ${item.talla || '-'} - Color ${item.color || '-'} - Cant ${item.cant} - Precio S/ ${totalItem.toFixed(2)}<br>`;
    previewHtml += `----------------------------------------------------------------------<br>`;
  });
